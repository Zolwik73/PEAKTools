...
namespace PCANBasicExample 
{
	...
	static int GetLengthFromDLC(int dlc)
	{
        if (dlc <= 8)
            return dlc;
		...
		private: String^ GetDataString()
            {
				...
				if ((m_Msg.MSGTYPE & TPCANMessageType::PCAN_MESSAGE_RTR) == TPCANMessageType::PCAN_MESSAGE_RTR)
					...
					for (int i = 0; i < GetLengthFromDLC(m_Msg.DLC); i++)
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->txtBitrate->Size = System::Drawing::Size(429, 34);
			...
			this->cbbParameter->Items->AddRange(gcnew cli::array< System::Object^  >(25) {
				L"Device ID", L"5V Power", L"Auto-reset on BUS-OFF",
					...
					L"Channel Capabilities", L"Bit rate Adaptation", L"Get Bit rate Nominal Information", L"Get Bit rate FD Information", L"Get CAN Nominal Speed Bit/s",
					L"Get CAN FD Speed Bit/s", L"Get IP Address", L"Get LAN Service Status", L"Reception of Status Frames", L"Reception of RTR Frames",
					L"Reception of Error Frames", L"Interframe Transmit Delay", L"Reception of Echo Frames", L"Hard Reset Status", L"Communication Direction",
					L"Global Unique Identifier (GUID)"
			});
			...
			this->nudLength->Maximum = System::Decimal(gcnew cli::array< System::Int32 >(4) { 15, 0, 0, 0 });
			...
			this->chbBRS->Location = System::Drawing::Point(567, 63);
			...
			this->Text = L"PCAN-Basic FD Sample";
...
#pragma region Message-proccessing functions
	...
	/// <summary>
	/// Function for writing messages on FD devices
	/// </summary>
	/// <returns>A TPCANStatus error code</returns>
	private: TPCANStatus^ WriteFrameFD()
	{
		...
		CANMsg->MSGTYPE = CANMsg->MSGTYPE | TPCANMessageType::PCAN_MESSAGE_FD;
	...
	private: void InsertMsgEntry(TPCANMsgFD ^newMsg, TPCANTimestampFD ^timeStamp)
	{
		...
		try
		{
            ...
            //
			lviCurrentItem->SubItems->Add(GetLengthFromDLC(newMsg->DLC).ToString());
		...
		try
		{
            ...
            for each (MessageStatus^ msgStatus in m_LastMsgsList)
            {
                ...
                if (msgStatus->MarkedAsUpdated)
                {
					...
					lviCurrentItem->SubItems[2]->Text = GetLengthFromDLC(msgStatus->CANMsg.DLC).ToString();
...
#pragma region Event Handlers
	private: System::Void cbbChannel_SelectedIndexChanged(System::Object^  sender, System::EventArgs^  e)
	{
		...
		strTemp = strTemp->Replace(L'h', L' ')->Trim(L' ');
				...
				if ((*stsResult) == TPCANStatus::PCAN_ERROR_OK) 
					for each (TPCANChannelInformation ^channel in info)
						if ((channel->channel_condition & PCANBasic::PCAN_CHANNEL_AVAILABLE) == PCANBasic::PCAN_CHANNEL_AVAILABLE)
						{
							...
							// Checks whether the current channel is available and if it supports the FD protocol
							if ((channel->channel_condition & PCANBasic::PCAN_CHANNEL_AVAILABLE) == PCANBasic::PCAN_CHANNEL_AVAILABLE &&
								(protocol & CANProtocol::CAN_FD) == CANProtocol::CAN_FD)
							{
	...
	private: System::Void chbExtended_CheckedChanged(System::Object^  sender, System::EventArgs^  e) 
	{
		...
		// the only way that the text length can be bigger als max length
	...
	private: System::Void cbbParameter_SelectedIndexChanged(System::Object^  sender, System::EventArgs^  e) 
	{
		...
		laDeviceOrDelay->Text = (cbbParameter->SelectedIndex == 20) ? "Delay (\u00B5s):" : "Device ID (Hex):";
	...
	private: System::Void nudLength_ValueChanged(System::Object^  sender, System::EventArgs^  e) 
	{
		...
		iLength = GetLengthFromDLC((int)nudLength->Value);
