...
/**
 * The TableMessageThread class extends Thread class and is used to process
 * JTable display.
 */
public class TableMessageThread extends Thread {
    ...
    private HashMap<Long, TableDataRow> data;
     ...
     */
    public TableMessageThread(JTable table, HashMap<Long, TableDataRow> data) {
        this.table = table;
        this.data = data;
    }
    ...
    public void run() {
        ...
        HashMap<Long, TableDataRow> datatmp;
        ...
        while (true) {
            ...
            synchronized (Application.token) {
                datatmp = new HashMap<Long, TableDataRow>(data);
            }
            for (Object item : datatmp.values()) {
                ...
                //Get PID
                msgPID = Integer.toHexString(dataRow.getMsgPID()) + "h";
                //Get Length
                msgLength = String.valueOf(dataRow.getMsgLength());
                //Get SDT
                msgSDT = Integer.toHexString(dataRow.getMsgSDT()) + "h";
                //Get VCID
                msgVCID = Integer.toHexString(dataRow.getMsgVCID()) + "h";
                //Get AF
                msgAF = Integer.toHexString(dataRow.getMsgAF()) + "h";
                ...
                msgTableObect = new Object[]{msgType, msgPID, msgLength, msgSDT,
                    msgVCID, msgAF, msgCount, msgData, msgRcvTime};

                synchronized (Application.token) {
                    //Search Existing DataRow In Model
                    for (int i = 0; i < model.getRowCount(); i++) {
                        if (model.getValueAt(i, 1).toString().equals(msgPID)
                                && model.getValueAt(i, 0).toString().equals(msgType)) {
                            msgIndex = i;
                            break;
                        }
                    }
                    ...
                    if (msgIndex == -1) {
                        ...
                        for (int i = 0; i < msgTableObect.length; i++) {
