...
set REQUIRED_JAVA_EXE_NAME=%JAVAC_EXE%
...
"%JAVAC%" -cp "%LIB_PCANBASIC_JAR%;" -d %CLASS_DIR% -Xlint:unchecked @build_sources.txt
@echo off
@if not %ERRORLEVEL% == 0 (
    echo ERROR: "%JAVAC%" returned a non-zero error code...
    pause
) else (
