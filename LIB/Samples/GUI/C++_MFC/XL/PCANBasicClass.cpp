...
void PCANBasicClass::LoadAPI()
{
    ...
    // Loads API functions
    //
    m_pInitializeXL = (fpInitializeFD)GetFunction("CAN_InitializeXL");
    m_pInitializeFD = (fpInitializeXL)GetFunction("CAN_InitializeFD");
    ...
    m_pReadXL = (fpReadXL)GetFunction("CAN_ReadXL");
    m_pWriteFD = (fpWriteFD)GetFunction("CAN_WriteFD");
    m_pWriteXL = (fpWriteXL)GetFunction("CAN_WriteXL");
    ...
    m_bWasLoaded = m_pInitializeXL && m_pInitializeFD && m_pUnInitialize && m_pReset &&
        m_pGetStatus && m_pReadFD && m_pReadXL && m_pWriteFD && m_pFilterMessages &&
...
TPCANStatus PCANBasicClass::InitializeXL(
    TPCANHandle Channel,
    TPCANBitrateXL BitrateXL)
{
    ...
    return (TPCANStatus)m_pInitializeXL(Channel, BitrateXL);
}
...
TPCANStatus PCANBasicClass::ReadXL(
    TPCANHandle Channel,
    TPCANMsgXL*
    MessageBuffer,
    TPCANTimestampXL *TimestampBuffer)
{
    ...
    return (TPCANStatus)m_pReadXL(Channel, MessageBuffer, TimestampBuffer);
}
...
TPCANStatus PCANBasicClass::WriteXL(
    TPCANHandle Channel,
    TPCANMsgXL* MessageBuffer)
{
    ...
    return (TPCANStatus)m_pWriteXL(Channel, MessageBuffer);
}
