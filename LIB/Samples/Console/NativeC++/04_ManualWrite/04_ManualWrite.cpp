...
TPCANStatus ManualWrite::WriteMessageXL()
{
    // Sends a CAN-XL message with standard ID and 2048 data bytes
    TPCANMsgXL msgCanMessageXL{};
    ...
    msgCanMessageXL.MSGTYPE = PCAN_MESSAGE_XL;
    for (int i = 0; i < 2048; i++)
    {
        BYTE input = (BYTE) (i % 256);
        msgCanMessageXL.DATA[i] = input;
    }
    return CAN_WriteXL(PcanHandle, &msgCanMessageXL);
}
...
void ManualWrite::FormatChannelName(TPCANHandle handle, LPSTR buffer, CANProtocol protocol)
{
    BYTE byChannel;
