...
TPCANStatus ManualRead::ReadMessageXL()
{
    TPCANMsgXL CANMsg;
...
void ManualRead::ProcessMessageCanXL(TPCANMsgXL msg, TPCANTimestampXL itsTimeStamp)
{
    std::cout << "Type: " << GetMsgTypeStringXL(msg.MSGTYPE, msg.RRS, msg.SEC) << "\n";
...
void ManualRead::FormatChannelName(TPCANHandle handle, LPSTR buffer, CANProtocol protocol)
{
    BYTE byChannel;
...
std::string ManualRead::GetMsgTypeStringXL(TPCANMessageTypeXL msgType, bool rrsFlag, bool secFlag)
{
    std::string strTemp;
...
std::string ManualRead::GetIdString(UINT32 id, TPCANMessageTypeXL msgType) 
{
    char result[MAX_PATH] = { 0 };
    if ((msgType & PCAN_MESSAGE_EXTENDED) == PCAN_MESSAGE_EXTENDED)
    {
...
int ManualRead::GetLengthFromDLC(WORD dlc)
{
    if (dlc < 0)
        return 1;
...
std::string ManualRead::GetDataString(BYTE data[], TPCANMessageTypeXL msgType, int dataLength)
{
    if ((msgType & PCAN_MESSAGE_RTR) == PCAN_MESSAGE_RTR)
        return "Remote Request";
    else
...
std::string ManualRead::GetDataString(BYTE  data[], int dataLength)
{
