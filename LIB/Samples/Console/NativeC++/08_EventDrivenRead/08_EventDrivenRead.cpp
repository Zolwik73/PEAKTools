...
TPCANStatus EventDrivenRead::ReadMessageXL()
{
    TPCANMsgXL CANMsg;
    TPCANTimestampXL CANTimeStamp;

    // We execute the "Read" function of the PCANBasic    
    TPCANStatus stsResult = CAN_ReadXL(PcanHandle, &CANMsg, &CANTimeStamp);
...
void EventDrivenRead::ProcessMessageCan(TPCANMsg msg, TPCANTimestamp itsTimeStamp)
{
    ...
    std::cout << "\nType: " << GetMsgTypeString(msg.MSGTYPE) << "\n";
...
void EventDrivenRead::ProcessMessageCanXL(TPCANMsgXL msg, TPCANTimestampXL itsTimeStamp)
{
    std::cout << "\nType: " << GetMsgTypeStringXL(msg.MSGTYPE, msg.RRS, msg.SEC) << "\n";
    std::cout << "PID: " << GetIdString(msg.PID, msg.MSGTYPE) << "\n";
    std::cout << "Length: " << GetLengthFromDLC(msg.DLC) << "\n";
    std::cout << "Time: " << GetTimeString(itsTimeStamp) << "\n";
    std::cout << "Data: " << GetDataString(msg.DATA, msg.MSGTYPE, GetLengthFromDLC(msg.DLC)) << "\n";
    std::cout << "----------------------------------------------------------";
...
void EventDrivenRead::FormatChannelName(TPCANHandle handle, LPSTR buffer, CANProtocol protocol)
{
    BYTE byChannel;
...
std::string EventDrivenRead::GetMsgTypeStringXL(TPCANMessageTypeXL msgType, bool rrsFlag, bool secFlag)
{
    std::string strTemp;
    bool isEcho = (msgType & PCAN_MESSAGE_ECHO) == PCAN_MESSAGE_ECHO;
    ...
    if ((msgType & PCAN_MESSAGE_XL) == PCAN_MESSAGE_XL)
        strTemp = "XL";
    else
        if ((msgType & PCAN_MESSAGE_EXTENDED) == PCAN_MESSAGE_EXTENDED)
            strTemp = "EXT";
    ...
    else
    {
        ...
        if ((int)msgType >= PCAN_MESSAGE_XL)
        {
            if (rrsFlag)
                strTemp += " RRS";
            if (secFlag)
        ...
        else if ((int)msgType > (int)PCAN_MESSAGE_EXTENDED)
        {
            if (isEcho)
                strTemp += " [ ECHO";
            else
                strTemp += " [ ";
            if ((msgType & PCAN_MESSAGE_FD) == PCAN_MESSAGE_FD)
                strTemp += " FD";
            if ((msgType & PCAN_MESSAGE_BRS) == PCAN_MESSAGE_BRS)
                strTemp += " BRS";
            if ((msgType & PCAN_MESSAGE_ESI) == PCAN_MESSAGE_ESI)
                strTemp += " ESI";
            strTemp += " ]";
        }
    }
...
std::string EventDrivenRead::GetIdString(UINT32 id, TPCANMessageTypeXL msgType)
{
    char result[MAX_PATH] = { 0 };
    if ((msgType & PCAN_MESSAGE_EXTENDED) == PCAN_MESSAGE_EXTENDED)
    {
...
int EventDrivenRead::GetLengthFromDLC(WORD dlc)
{
    if (dlc < 0)
        return 1;
...
std::string EventDrivenRead::GetDataString(BYTE data[], TPCANMessageTypeXL msgType, int dataLength)
{
    if ((msgType & PCAN_MESSAGE_RTR) == PCAN_MESSAGE_RTR)
        return "Remote Request";
    else
...
std::string EventDrivenRead::GetDataString(BYTE  data[], int dataLength)
{
