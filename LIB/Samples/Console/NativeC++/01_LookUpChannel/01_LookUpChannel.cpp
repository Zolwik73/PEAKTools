...
LookUpChannel::LookUpChannel()
{
    ...
    if (DeviceGUID != "")
    {
        if (sParameters != "")
            sprintf_s(sParameters, sizeof(sParameters), "%s, ", sParameters);
        sprintf_s(sParameters, sizeof(sParameters), "%s%s=%s", sParameters, ConvertDefinesToChar((wchar_t*)LOOKUP_DEVICE_GUID), DeviceGUID);
    }
    ...
    if (stsResult == PCAN_ERROR_OK)
    {
        ...
        if (handle != PCAN_NONEBUS)
        {
            ...
            if (stsResult == PCAN_ERROR_OK)
            {
                ...
                bool isFD = (iFeatures & FEATURE_FD_CAPABLE) == FEATURE_FD_CAPABLE;
                bool isXL = (iFeatures & FEATURE_XL_CAPABLE) == FEATURE_XL_CAPABLE;
                FormatChannelName(handle, buffer, GetProtocol(isFD, isXL));
                std::cout << "The channel handle " << buffer << " was found\n";
            }
            else
                std::cout << "There was an issue retrieving supported channel features\n";
        }
        else
...
void LookUpChannel::ShowConfigurationHelp()
{
    ...
    std::cout << "* DeviceGUID: String value that represents a valid GUID for a PCAN device                |\n";
    std::cout << "                                                                                         |\n";
...
void LookUpChannel::ShowCurrentConfiguration()
{
    ...
    std::cout << "* DeviceGUID: " << DeviceGUID << "\n";
    std::cout << "\n";
}
...
void LookUpChannel::FormatChannelName(TPCANHandle handle, LPSTR buffer, CANProtocol protocol)
{
    BYTE byChannel;
...
CANProtocol LookUpChannel::GetProtocol(bool isFD, bool isXL)
{
    if (!isFD && !isXL)
