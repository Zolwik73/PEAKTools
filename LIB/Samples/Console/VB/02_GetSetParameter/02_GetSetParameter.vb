...
#Region "PCAN_DEVICE_GUID"
	...
	''' </summary>
	Private Sub GetPCAN_DEVICE_GUID()
		Dim stsResult As TPCANStatus
		Dim strBuffer As StringBuilder = New StringBuilder(256)
		stsResult = PCANBasic.GetValue(PcanHandle, TPCANParameter.PCAN_DEVICE_GUID, strBuffer, 256)

		If stsResult = TPCANStatus.PCAN_ERROR_OK Then
			Console.WriteLine("-----------------------------------------------------------------------------------------")
			Console.WriteLine("Get PCAN_DEVICE_GUID: " + strBuffer.ToString)
			Console.WriteLine()
		Else
			ShowStatus(stsResult)
		End If
...
#Region "Help-Functions"
	...
	Private Function ConvertToChannelFeatures(ByVal value As UInteger) As String
		...
		End If
		If (value And PCANBasic.FEATURE_XL_CAPABLE) = PCANBasic.FEATURE_XL_CAPABLE Then
			If sFeatures <> "" Then
