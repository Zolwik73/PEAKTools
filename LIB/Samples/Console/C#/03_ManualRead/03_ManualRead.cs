	...
	public class ManualReadSample
	{
		...
		public ManualReadSample()
		{
			...
			do
			{
				Console.Clear();
				ReadMessages();
				Console.WriteLine("Do you want to read again? yes[y] or any other key to close");
		...
		private TPCANStatus ReadMessageXL()
		{
			...
			TPCANStatus stsResult = PCANBasic.ReadXL(PcanHandle, out TPCANMsgXL CANMsg, out TPCANTimestampXL CANTimeStamp);
		...
		/// <param name="itsTimeStamp">Timestamp of the message as microseconds (ulong)</param>
		private void ProcessMessageCanXl(TPCANMsgXL msg, TPCANTimestampXL itsTimeStamp)
		{
		...
		{
			Console.WriteLine("=========================================================================================");
			Console.WriteLine("|                           PCAN-Basic ManualRead Example                                |");
			Console.WriteLine("=========================================================================================");
		...
		private string GetMsgTypeStringXL(TPCANMessageTypeXL msgType, bool rrsFlag, bool secFlag)
		{
			string strTemp;
			...
			else
			{
				...
				if ((int)msgType >= (uint)TPCANMessageTypeXL.PCAN_MESSAGE_XL)
				{
					if (rrsFlag)
						strTemp += " RRS";
					if (secFlag)
		...
		private string GetIdString(uint id, TPCANMessageTypeXL msgType)
		{
			if ((msgType & TPCANMessageTypeXL.PCAN_MESSAGE_EXTENDED) == TPCANMessageTypeXL.PCAN_MESSAGE_EXTENDED)
				return string.Format("{0:X8}h", id);
		...
		public static int GetLengthFromDLC(ushort dlc)
		{
			if (dlc < 0)
				return 1;
		...
		private string GetTimeString(ulong time)
		{
		...
		/// <param name="dataLength">The amount of bytes to take into account within the given data</param>
		/// <returns>A string with hexadecimal formatted data bytes of a CAN message</returns>
		private string GetDataString(byte[] data, TPCANMessageTypeXL msgType, int dataLength)
		{
			if ((msgType & TPCANMessageTypeXL.PCAN_MESSAGE_RTR) == TPCANMessageTypeXL.PCAN_MESSAGE_RTR)
				return "Remote Request";
			else
		...
		/// <param name="dataLength">The amount of bytes to take into account within the given data</param>
		/// <returns>A string with hexadecimal formatted data bytes of a CAN message</returns>
		private string GetDataString(byte[] data, int dataLength)
		{
			string strTemp = "";
