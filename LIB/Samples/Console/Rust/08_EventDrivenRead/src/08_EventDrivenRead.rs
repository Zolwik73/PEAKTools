...
pub fn run() {
    ...
    let read_thread = thread::spawn(move || {
        let receive_event = match unsafe { CreateEventW(None, false, false, None) } {
            Ok(evt) => evt,
            Err(_) => {
                println!("Could not create receive event object.");
        ...
        let mut buffer: u64 = receive_event.0 as u64;
        if let Err(sts) = SetValueInt64(PCAN_HANDLE, PCAN_RECEIVE_EVENT, &buffer) {
            show_status(sts);
            let _ = unsafe { CloseHandle(receive_event) };
            let _ = Uninitialize(PCAN_NONEBUS);
            return;
        }
        ...
        while running_thread.load(Ordering::Relaxed) {
            ...
            let wait_result = unsafe { WaitForSingleObject(receive_event, 50) };
...
fn read_message_xl() -> std::result::Result<(), TPCANStatus> {
...
fn read_message_fd() -> std::result::Result<(), TPCANStatus> {
    // We execute the "Read" function of the PCANBasic
...
fn read_message() -> std::result::Result<(), TPCANStatus> {
    // We execute the "Read" function of the PCANBasic
...
fn process_message_can(msg: TPCANMsg, timestamp: TPCANTimestamp) {
    ...
    println!("Data: {}", get_data_string(&msg.DATA, msg.LEN as usize));
    println!("----------------------------------------------------------");
}
...
fn process_message_can_fd(msg: TPCANMsgFD, timestamp: TPCANTimestampFD) {
    ...
    println!("Data: {}", get_data_string(&msg.DATA, length));
    println!("----------------------------------------------------------");
}
...
fn show_configuration_help() {
    println!("=========================================================================================");
    println!("|                           PCAN-Basic EventDrivenRead Example                           |");
    println!("=========================================================================================");
...
fn get_msg_type_string(msg_type: TPCANMessageType) -> &'static str {
...
fn get_data_string(data: &[u8], data_length: usize) -> String {
    data.iter()
        .take(data_length)
        .map(|b| format!("{:02X} ", b))
        .collect::<String>()
