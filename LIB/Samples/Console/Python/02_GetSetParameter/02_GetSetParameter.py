...
class GetSetParameter():
    ...
    def GetPCAN_HARD_RESET_STATUS(self):
        """
        Shows the status of the hard reset within the PCANBasic.Reset method
        """
        stsResult = self.m_objPCANBasic.GetValue(self.PcanHandle,PCAN_HARD_RESET_STATUS)

        if stsResult[0] == PCAN_ERROR_OK:
            print("-----------------------------------------------------------------------------------------")
            print("Get PCAN_HARD_RESET_STATUS: " + self.ConvertToParameterOnOff(stsResult[1]))
            print("")
        else:
            self.ShowStatus(stsResult)

    def SetPCAN_HARD_RESET_STATUS(self,boolValue):
        """
        ...
        if type(boolValue) == bool:
            ...
            stsResult = self.m_objPCANBasic.SetValue(self.PcanHandle,PCAN_HARD_RESET_STATUS,iHardResetStatus)

            if stsResult == PCAN_ERROR_OK:
                print("-----------------------------------------------------------------------------------------")
                print("Set PCAN_HARD_RESET_STATUS: " + self.ConvertToParameterOnOff(iHardResetStatus))
                print("")
            else:
                self.ShowStatus(stsResult)
        else:
            print("-----------------------------------------------------------------------------------------")
            print("Warning! Set PCAN_HARD_RESET_STATUS wrong value type: " + str(type(boolValue)))
            print("")
    
    def GetPCAN_LAN_CHANNEL_DIRECTION(self):
        """
        Shows the communication direction of a PCAN-Channel representing a LAN interface
        """
        stsResult = self.m_objPCANBasic.GetValue(self.PcanHandle,PCAN_LAN_CHANNEL_DIRECTION)

        if stsResult[0] == PCAN_ERROR_OK:
            print("-----------------------------------------------------------------------------------------")
            print("Get PCAN_LAN_CHANNEL_DIRECTION: " + self.ConvertToChannelDirection(stsResult[1]))
            print("")
        else:
            self.ShowStatus(stsResult)        

    def GetPCAN_DEVICE_GUID(self):
        """
        Shows the global unique identifier associated to a PCAN device
        """
        stsResult = self.m_objPCANBasic.GetValue(self.PcanHandle,PCAN_DEVICE_GUID)

        if stsResult[0] == PCAN_ERROR_OK:
            print("-----------------------------------------------------------------------------------------")
            print("Get PCAN_DEVICE_GUID: " + stsResult[1].decode("utf-8"))
            print("")
        else:
            self.ShowStatus(stsResult) 
    ...
    def ConvertToChannelDirection(self,value):
        """
        Convert uint value to readable string value
        """
        switcher = {LAN_DIRECTION_READ:"incoming only",
                LAN_DIRECTION_WRITE:"outgoing only",
                LAN_DIRECTION_READ_WRITE:"bidirectional"}
    
        if value in switcher:
            return switcher[value]
        else:
            return "undefined (0x{0:X4})".format(value)
