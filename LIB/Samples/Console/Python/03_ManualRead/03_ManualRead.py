...
class ManualRead():
    ...
    def __init__(self):
        ...
        while strinput == "y":
            self.clear()
            self.ReadMessages()
            strinput = self.getInput("Do you want to read again? yes[y] or any other key to exit...", "y")
    ...
    def ReadMessages(self):
        ...
        ## find more. If the queue is empty or an error occur, we get out from the while statement.
    ...
    def ReadMessageXL(self):
        ...
        """
        ## We execute the "Read" function of the PCANBasic
        stsResult = self.m_objPCANBasic.ReadXL(self.PcanHandle)

        if stsResult[0] == PCAN_ERROR_OK:
            ## We show the received message
            self.ProcessMessageCanXl(stsResult[1],stsResult[2])
    ...
    def ProcessMessageCan(self,msg,itstimestamp):
         ...
         print("Length: " + str(self.GetLengthFromDLC(msg.LEN)))
         print("Time: " + self.GetTimeString(microsTimeStamp))
         print("Data: " + self.GetDataString(msg.DATA, msg.MSGTYPE, self.GetLengthFromDLC(msg.LEN)))
         print("----------------------------------------------------------")
    ...
    def ProcessMessageCanFd(self,msg,itstimestamp):
        ...
        print("Time: " + self.GetTimeString(itstimestamp.value))
        print("Data: " + self.GetDataString(msg.DATA, msg.MSGTYPE, self.GetLengthFromDLC(msg.DLC)))
        print("----------------------------------------------------------")
        
    def ProcessMessageCanXl(self,msg,itstimestamp):
        """
        Processes a received CAN-XL message

        Parameters:
            msg = The received PCAN-Basic CAN-XL message
            itstimestamp = Timestamp of the message as microseconds (ulong)
        """        
        print("Type: " + self.GetTypeStringXL(msg.MSGTYPE, msg.RRS, msg.SEC))
        print("PID: " + self.GetIdString(msg.PID, msg.MSGTYPE))
        print("Length: " + str(self.GetLengthFromDLC_XL(msg.DLC)))
        print("Time: " + self.GetTimeString(itstimestamp.value))
        print("Data: " + self.GetDataString(msg.DATA, msg.MSGTYPE, self.GetLengthFromDLC_XL(msg.DLC)))
        print("----------------------------------------------------------")
    #endregion
    ...
    def GetLengthFromDLC_XL(self, dlc):
        """
        Converts a CAN DLC value into the actual data length of the CAN-XL frame.

        dlc = A value between 0 and 2047 (CAN XL DLC range)
    ...
    def GetIdString(self, id, msgtype: TPCANMessageType):
        """
    ...
    def GetTypeStringXL(self, msgtype, rrsflag, secflag):
        """
        Gets the string representation of the type of a CAN XL message

        Parameters:
            msgtype = Type of a CAN XL message
            rrsflag = The RRS flag of the XL message
            secflag = The SEC flag of the XL message
        
        Returns: 
            The type of the CAN XL message as string
        """
        isEcho = (msgtype & PCAN_MESSAGE_ECHO.value) == PCAN_MESSAGE_ECHO.value
        ...
        if (msgtype & PCAN_MESSAGE_XL.value) == PCAN_MESSAGE_XL.value:
            strTemp = "XL"
        elif (msgtype & PCAN_MESSAGE_EXTENDED.value) == PCAN_MESSAGE_EXTENDED.value:
            strTemp = "EXT"
        else:
            strTemp = "STD"

        if (msgtype & PCAN_MESSAGE_RTR.value) == PCAN_MESSAGE_RTR.value:
            strTemp += "/RTR [ ECHO ]" if isEcho else "/RTR"
        else:
            if isEcho:
                strTemp += " [ ECHO"
            else:
            ...
            if int(msgtype) >= int(PCAN_MESSAGE_XL.value):
                if rrsflag:
                    strTemp += " RRS"
                if secflag:
                    strTemp += " SEC"
                strTemp += " ]"
            elif int(msgtype) > int(PCAN_MESSAGE_EXTENDED.value):
                if (msgtype & PCAN_MESSAGE_FD.value) == PCAN_MESSAGE_FD.value:
                    strTemp += " FD"
                if (msgtype & PCAN_MESSAGE_BRS.value) == PCAN_MESSAGE_BRS.value:
                    strTemp += " BRS"
    ...
    def GetDataString(self, data, msgtype, dataLength):
        """
