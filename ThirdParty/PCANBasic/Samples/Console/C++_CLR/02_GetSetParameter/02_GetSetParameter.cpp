...
#pragma region PCAN_DEVICE_GUID
    /// <summary>
    /// Shows the global unique identifier associated to a PCAN device
    /// </summary>
    void GetPCAN_DEVICE_GUID()
    {
        StringBuilder^ strBuffer = gcnew StringBuilder(255);
        TPCANStatus stsResult = PCANBasic::GetValue(PcanHandle, TPCANParameter::PCAN_DEVICE_GUID, strBuffer, 256);

        if (stsResult == TPCANStatus::PCAN_ERROR_OK)
        {
            Console::WriteLine("-----------------------------------------------------------------------------------------");
            Console::WriteLine("Get PCAN_DEVICE_GUID: " + strBuffer);
            Console::WriteLine();
        }
        else
            ShowStatus(stsResult);
...
private:
    ...
    String^ ConvertToChannelFeatures(UInt32 value)
    {
        ...
        if ((value & PCANBasic::FEATURE_XL_CAPABLE) == PCANBasic::FEATURE_XL_CAPABLE)
            if (sFeatures != "")
