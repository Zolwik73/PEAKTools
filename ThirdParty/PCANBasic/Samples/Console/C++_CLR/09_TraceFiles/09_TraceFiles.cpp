...
private:
    ...
    /// </summary>
    const CANProtocol Protocol = CANProtocol::CAN_XL;
    /// <summary>
    ...
    /// </summary>
    literal String^ TracePath = "";
    /// <summary>
    /// Timerinterval (ms) for reading 
    /// </summary>
    literal int TimerInterval = 250;
#pragma endregion
...
private:
    ...
    void SetTimer()
    {
        m_Timer = gcnew Timer(TimerInterval);
        m_Timer->Elapsed += gcnew ElapsedEventHandler(this, &TraceFiles::OnTimedEvent); // Hook up the Elapsed event for the timer. 
    ...
    void ReadMessages()
    {
        ...
        {
            if (Protocol == CANProtocol::CAN_XL)
                stsResult = PCANBasic::ReadXL(PcanHandle, CANMsgXL);
            else if (Protocol == CANProtocol::CAN_FD)
    ...
    void StopTrace()
    {
        UInt32 iStatus = PCANBasic::PCAN_PARAMETER_OFF;

        TPCANStatus stsResult = PCANBasic::SetValue(PcanHandle, TPCANParameter::PCAN_TRACE_STATUS, iStatus, sizeof(UInt32)); // We stop the tracing by setting the parameter.
    ...
    bool ConfigureTrace()
    {
        ...
        TPCANStatus stsResult = PCANBasic::SetValue(PcanHandle, TPCANParameter::PCAN_TRACE_LOCATION, TracePath, sizeof(UInt32));

        if (stsResult == TPCANStatus::PCAN_ERROR_OK)
        {
            // Sets the maximum size of a tracefile 
            stsResult = PCANBasic::SetValue(PcanHandle, TPCANParameter::PCAN_TRACE_SIZE, iSize, sizeof(UInt32));
            ...
            if (stsResult == TPCANStatus::PCAN_ERROR_OK)
            {
                ...
                // Sets the config
                stsResult = PCANBasic::SetValue(PcanHandle, TPCANParameter::PCAN_TRACE_CONFIGURE, config, sizeof(int));
    ...
    bool StartTrace()
    {
        UInt32 iStatus = PCANBasic::PCAN_PARAMETER_ON;

        TPCANStatus stsResult = PCANBasic::SetValue(PcanHandle, TPCANParameter::PCAN_TRACE_STATUS, iStatus, sizeof(UInt32)); // We activate the tracing by setting the parameter.
...
{
    TraceFiles^ start = gcnew TraceFiles();
    return 0;
}
