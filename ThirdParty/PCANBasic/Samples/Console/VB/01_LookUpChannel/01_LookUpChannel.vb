	...
	Sub Main()
		...
		If stsResult = TPCANStatus.PCAN_ERROR_OK Then
			If handle <> PCANBasic.PCAN_NONEBUS Then
				...
				If stsResult = TPCANStatus.PCAN_ERROR_OK Then
					Dim isFD As Boolean = (iFeatures And PCANBasic.FEATURE_FD_CAPABLE) = PCANBasic.FEATURE_FD_CAPABLE
					Dim isXL As Boolean = (iFeatures And PCANBasic.FEATURE_XL_CAPABLE) = PCANBasic.FEATURE_XL_CAPABLE
					Console.WriteLine("The channel handle " + FormatChannelName(handle, GetProtocol(isFD, isXL)) + " was found")
				Else
...
#Region "Help-Functions"
	...
	Private Function GetProtocol(ByVal isFD As Boolean, ByVal isXL As Boolean) As CANProtocol
		If Not isFD And Not isXL Then
			Return CANProtocol.CAN
		ElseIf isFD And Not isXL Then
			Return CANProtocol.CAN_FD
		ElseIf Not isFD And isXL Then
			Return CANProtocol.CAN_XL
		ElseIf isFD And isXL Then
			Return CANProtocol.CAN_XL
		Else
			Return CANProtocol.CAN
		End If
