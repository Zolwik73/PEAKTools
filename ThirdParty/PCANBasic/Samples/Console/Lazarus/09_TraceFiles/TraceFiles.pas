...
begin
  ...
  stsResult := TPCANBasic.SetValue(FPcanHandle,TPCANParameter.PCAN_TRACE_STATUS,PLongWord(@iStatus),SizeOf(uint32));
...
begin
  ...
  stsResult := TPCANBasic.SetValue(FPcanHandle,TPCANParameter.PCAN_TRACE_LOCATION,PLongWord(@FTracePath),SizeOf(uint32));

  if (stsResult = TPCANStatus.PCAN_ERROR_OK) then
  begin
    // Sets the maximum size of a tracefile
    stsResult := TPCANBasic.SetValue(FPcanHandle,TPCANParameter.PCAN_TRACE_SIZE,PLongWord(@iSize),SizeOf(uint32));
    ...
    begin
      ...
      // Sets the config
      stsResult := TPCANBasic.SetValue(FPcanHandle,TPCANParameter.PCAN_TRACE_CONFIGURE,PLongWord(@config),SizeOf(uint32));
