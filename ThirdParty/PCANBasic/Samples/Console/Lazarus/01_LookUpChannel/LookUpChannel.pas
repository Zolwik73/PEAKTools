...
begin
  ...
  if (IsStringEmpty(FDeviceGUID)) then
    begin
    if (IsStringEmpty(sParameters)) then
      sParameters := Format('%s, ',[sParameters]);
    sParameters := Format('%s%s=%s',[sParameters,TPCANBasic.LOOKUP_DEVICE_GUID,FDeviceGUID]);
    end;    
  ...
  begin
    ...
    begin
      stsResult := TPCANBasic.GetValue(handle,TPCANParameter.PCAN_CHANNEL_FEATURES,PLongWord(@iFeatures),SizeOf(iFeatures));
...
begin
  ...
  writeln('* FDeviceGUI: String value that represents a valid GUID for a PCAN device                 |');
  writeln('                                                                                          |');
...
begin
  ...
  writeln('* FDeviceGUID: '+ FDeviceGUID);
  writeln();
end;
