...
var
  ...
  iDeviceID: Integer;
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_DEVICE_ID, PLongWord(@iDeviceID),SizeOf(iDeviceID));
...
var
  stsResult:TPCANStatus;
begin
  stsResult := TPCANBasic.SetValue(FPcanHandle, TPCANParameter.PCAN_DEVICE_ID, PLongWord(@iDeviceID),SizeOf(iDeviceID));
...
begin
  ...
  stsResult := TPCANBasic.GetValue(TPCANBasic.PCAN_NONEBUS, TPCANParameter.PCAN_ATTACHED_CHANNELS_COUNT, PLongWord(@iChannelsCount), SizeOf(iChannelsCount));

  if (stsResult = TPCANStatus.PCAN_ERROR_OK) then
  begin
    SetLength(ciChannelInformation, iChannelsCount);

    stsResult := TPCANBasic.GetValue(TPCANBasic.PCAN_NONEBUS, TPCANParameter.PCAN_ATTACHED_CHANNELS, PTPCANChannelInformation(ciChannelInformation), SizeOf(TPCANChannelInformation) * iChannelsCount);
    if (stsResult = TPCANStatus.PCAN_ERROR_OK) then
    begin
      writeln('-----------------------------------------------------------------------------------------');
...
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_CHANNEL_CONDITION, PLongWord(@iChannelCondition),SizeOf(iChannelCondition));
...
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_CHANNEL_IDENTIFYING, PLongWord(@iChannelIdentifying),SizeOf(iChannelIdentifying));
...
begin
  ...
  stsResult := TPCANBasic.SetValue(FPcanHandle, TPCANParameter.PCAN_CHANNEL_IDENTIFYING, PLongWord(@iChannelIdentifying),SizeOf(iChannelIdentifying));
...
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_CHANNEL_FEATURES, PLongWord(@iChannelFeatures),SizeOf(iChannelFeatures));
...
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_BITRATE_ADAPTING, PLongWord(@iBitrateAdapting),SizeOf(iBitrateAdapting));
...
begin
  ...
  stsResult := TPCANBasic.SetValue(FPcanHandle, TPCANParameter.PCAN_BITRATE_ADAPTING, PLongWord(@iBitrateAdapting),SizeOf(iBitrateAdapting));
...
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_ALLOW_STATUS_FRAMES, PLongWord(@iAllowStatusFrames),SizeOf(iAllowStatusFrames));
...
begin
  ...
  stsResult := TPCANBasic.SetValue(FPcanHandle, TPCANParameter.PCAN_ALLOW_STATUS_FRAMES, PLongWord(@iAllowStatusFrames),SizeOf(iAllowStatusFrames));
...
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_ALLOW_RTR_FRAMES, PLongWord(@iAllowRTRFrames),SizeOf(iAllowRTRFrames));
...
begin
  ...
  stsResult := TPCANBasic.SetValue(FPcanHandle, TPCANParameter.PCAN_ALLOW_RTR_FRAMES, PLongWord(@iAllowRTRFrames),SizeOf(iAllowRTRFrames));
...
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_ALLOW_ERROR_FRAMES, PLongWord(@iAllowErrorFrames),SizeOf(iAllowErrorFrames));
...
begin
  ...
  stsResult := TPCANBasic.SetValue(FPcanHandle, TPCANParameter.PCAN_ALLOW_ERROR_FRAMES, PLongWord(@iAllowErrorFrames),SizeOf(iAllowErrorFrames));
...
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_ALLOW_ECHO_FRAMES, PLongWord(@iAllowEchoFrames),SizeOf(iAllowEchoFrames));
...
begin
  ...
  stsResult := TPCANBasic.SetValue(FPcanHandle, TPCANParameter.PCAN_ALLOW_ECHO_FRAMES, PLongWord(@iAllowEchoFrames),SizeOf(iAllowEchoFrames));
...
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_ACCEPTANCE_FILTER_11BIT, PLongWord(@iacceptancefilter11bit),SizeOf(iacceptancefilter11bit));
...
var
  stsResult:TPCANStatus;
begin
  stsResult := TPCANBasic.SetValue(FPcanHandle, TPCANParameter.PCAN_ACCEPTANCE_FILTER_11BIT, PLongWord(@iacceptancefilter11bit),SizeOf(iacceptancefilter11bit));
...
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_ACCEPTANCE_FILTER_29BIT, PLongWord(@iAcceptanceFilter29Bit),SizeOf(iAcceptanceFilter29Bit));
...
var
  stsResult:TPCANStatus;
begin
  stsResult := TPCANBasic.SetValue(FPcanHandle, TPCANParameter.PCAN_ACCEPTANCE_FILTER_29BIT, PLongWord(@iacceptancefilter29bit),SizeOf(iacceptancefilter29bit));
...
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_MESSAGE_FILTER, PLongWord(@imessagefilter),SizeOf(imessagefilter));
...
var
  stsResult:TPCANStatus;
begin
  stsResult := TPCANBasic.SetValue(FPcanHandle, TPCANParameter.PCAN_MESSAGE_FILTER, PLongWord(@imessagefilter),SizeOf(imessagefilter));
...
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_HARD_RESET_STATUS, PLongWord(@iHardResetStatus),SizeOf(iHardResetStatus));
...
begin
  ...
  stsResult := TPCANBasic.SetValue(FPcanHandle, TPCANParameter.PCAN_HARD_RESET_STATUS, PLongWord(@iHardResetStatus),SizeOf(iHardResetStatus));
...
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_LAN_CHANNEL_DIRECTION, PLongWord(@iChannelDirection),SizeOf(iChannelDirection));
...
{$REGION 'PCAN_DEVICE_GUID'}
procedure TGetSetParameter.GetPCAN_DEVICE_GUID;
var
  stsResult:TPCANStatus;
  strBuffer :array[0..255] of Char;
begin
  stsResult := TPCANBasic.GetValue(FPcanHandle, TPCANParameter.PCAN_DEVICE_GUID, strBuffer, 256);

  if (stsResult = TPCANStatus.PCAN_ERROR_OK) then
  begin
    writeln('-----------------------------------------------------------------------------------------');
    writeln(Format('Get PCAN_DEVICE_GUID: %s', [strBuffer]));
    writeln('');
  end
  else
    ShowStatus(stsResult);
...
begin
  if ((handle >= TPCANBasic.PCAN_PCIBUS1) and (handle <= TPCANBasic.PCAN_PCIBUS8)) then
    Result := 'PCAN_PCI'
...
begin
  ...
  if ((value and TPCANBasic.FEATURE_XL_CAPABLE) = TPCANBasic.FEATURE_XL_CAPABLE) then
    if (sFeatures <> '') then
      sFeatures += ', FEATURE_XL_CAPABLE'
    else
      sFeatures += 'FEATURE_XL_CAPABLE';
