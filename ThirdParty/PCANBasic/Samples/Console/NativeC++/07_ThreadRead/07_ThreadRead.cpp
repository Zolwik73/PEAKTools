...
TPCANStatus ThreadRead::ReadMessageXL()
{
    TPCANMsgXL CANMsg;
...
void ThreadRead::ProcessMessageCanXL(TPCANMsgXL msg, TPCANTimestampXL itsTimeStamp)
{
...
void ThreadRead::FormatChannelName(TPCANHandle handle, LPSTR buffer, CANProtocol protocol)
{
    BYTE byChannel;
...
std::string ThreadRead::GetMsgTypeStringXL(TPCANMessageTypeXL msgType, bool rrsFlag, bool secFlag)
{
    std::string strTemp;
...
std::string ThreadRead::GetIdString(UINT32 id, TPCANMessageTypeXL msgType)
{
    char result[MAX_PATH] = { 0 };
    if ((msgType & PCAN_MESSAGE_EXTENDED) == PCAN_MESSAGE_EXTENDED)
    {
...
int ThreadRead::GetLengthFromDLC(WORD dlc)
{
    if (dlc < 0)
        return 1;
...
std::string ThreadRead::GetDataString(BYTE data[], TPCANMessageTypeXL msgType, int dataLength)
{
    if ((msgType & PCAN_MESSAGE_RTR) == PCAN_MESSAGE_RTR)
        return "Remote Request";
    else
...
std::string ThreadRead::GetDataString(BYTE  data[], int dataLength)
{
