...
GetSetParameter::GetSetParameter()
{
    ...
    if (Protocol == CANProtocol::CAN_XL)
        stsResult = CAN_InitializeXL(PcanHandle, BitrateXL);
    ...
    if (stsResult == PCAN_ERROR_OK)
    {
        ...
        if (stsResult == PCAN_ERROR_OK)
        {
            ...
            for (int i = 0; i < (int)iChannelsCount; i++)
            {
                ...
                BYTE byChannel;
                if (ciChannelInformation[i].channel_handle < 0x100)
                {
                    byChannel = (BYTE)(ciChannelInformation[i].channel_handle & 0xF);
                }
                else
                {
                    byChannel = (BYTE)(ciChannelInformation[i].channel_handle & 0xFF);
                }
                std::cout << "channel_handle:      " << buffer << "BUS" << static_cast<int>(byChannel) << "\n";
...
void GetSetParameter::SetPCAN_BITRATE_ADAPTING(bool value)
{
    ...
    if (Protocol == CANProtocol::CAN_XL)
        stsResult = CAN_InitializeXL(PcanHandle, BitrateFD);
    else if (Protocol == CANProtocol::CAN_FD)
        stsResult = CAN_InitializeFD(PcanHandle, BitrateFD);
    else
        stsResult = CAN_Initialize(PcanHandle, Bitrate);
...
void GetSetParameter::GetPCAN_HARD_RESET_STATUS()
{
    UINT32 iHardResetStatus;
    TPCANStatus stsResult = CAN_GetValue(PcanHandle, PCAN_HARD_RESET_STATUS, &iHardResetStatus, sizeof(UINT32));

    if (stsResult == PCAN_ERROR_OK)
    {
        std::cout << "-----------------------------------------------------------------------------------------\n";
        std::cout << "Get PCAN_HARD_RESET_STATUS: " << ConvertToParameterOnOff(iHardResetStatus) << "\n";
        std::cout << "\n";
    }
    else
        ShowStatus(stsResult);
...
void GetSetParameter::SetPCAN_HARD_RESET_STATUS(bool value)
{
    UINT32 iHardResetStatus;

    if (value)
        iHardResetStatus = PCAN_PARAMETER_ON;
    else
        iHardResetStatus = PCAN_PARAMETER_OFF;

    TPCANStatus stsResult = CAN_SetValue(PcanHandle, PCAN_HARD_RESET_STATUS, &iHardResetStatus, sizeof(UINT32));

    if (stsResult == PCAN_ERROR_OK)
    {
        std::cout << "-----------------------------------------------------------------------------------------\n";
        std::cout << "Set PCAN_HARD_RESET_STATUS: " << ConvertToParameterOnOff(iHardResetStatus) << "\n";
        std::cout << "\n";
    }
    else
        ShowStatus(stsResult);
...
void GetSetParameter::GetPCAN_LAN_CHANNEL_DIRECTION()
{
    UINT32 iChannelDirection;
    TPCANStatus stsResult = CAN_GetValue(PcanHandle, PCAN_LAN_CHANNEL_DIRECTION, &iChannelDirection, sizeof(UINT32));

    if (stsResult == PCAN_ERROR_OK)
    {
        std::cout << "-----------------------------------------------------------------------------------------\n";
        std::cout << "Get PCAN_LAN_CHANNEL_DIRECTION: " << ConvertToChannelDirection(iChannelDirection) << "\n";
        std::cout << "\n";
    }
    else
        ShowStatus(stsResult);
...
void GetSetParameter::GetPCAN_DEVICE_GUID()
{
    char strBuffer[256];
    TPCANStatus stsResult = CAN_GetValue(PcanHandle, PCAN_DEVICE_GUID, strBuffer, 256);

    if (stsResult == PCAN_ERROR_OK)
    {
        std::cout << "-----------------------------------------------------------------------------------------\n";
        std::cout << "Get PCAN_DEVICE_GUID: " << strBuffer << "\n";
        std::cout << "\n";
    }
    else
        ShowStatus(stsResult);
...
void GetSetParameter::ShowConfigurationHelp()
{
    ...
    std::cout << "* Protocol: Enum value that indicates the communication protocol, CAN, CAN-FD or CAN-XL. |\n";
    std::cout << "* Bitrate: Numeric value that represents the BTR0/BR1 bitrate value to be used for CAN   |\n";
    std::cout << "           communication                                                                 |\n";
    std::cout << "* BitrateFD: String value that represents the nominal/FD bitrate value to be used for    |\n";
    std::cout << "             CAN-FD communication                                                        |\n";
    std::cout << "* BitrateXL: String value that represents the nominal/FD/XL bitrate value to be used for |\n";
    std::cout << "             CAN-XL communication                                                        |\n";
    std::cout << "=========================================================================================\n";
    std::cout << "\n";
}
...
void GetSetParameter::ShowCurrentConfiguration()
{
    ...
    std::cout << "----------------------\n";
    char buffer[MAX_PATH];
    FormatChannelName(PcanHandle, buffer, Protocol);
    std::cout << "* PCANHandle: " << buffer << "\n";
    if (Protocol == CANProtocol::CAN_XL)
        std::cout << "* Protocol: CAN XL\n";
    else if (Protocol == CANProtocol::CAN_FD)
        std::cout << "* Protocol: CAN FD\n";
    else
        std::cout << "* Protocol: CAN\n";
    ConvertBitrateToString(Bitrate, buffer);
    std::cout << "* Bitrate: " << buffer << "\n";
    std::cout << "* BitrateFD: " << BitrateFD << "\n";
    std::cout << "* BitrateXL: " << BitrateXL << "\n";
    std::cout << "\n";
}
...
void GetSetParameter::FormatChannelName(TPCANHandle handle, LPSTR buffer, CANProtocol protocol)
{
    BYTE byChannel;
    ...
    if (protocol == CANProtocol::CAN_XL)
        sprintf_s(buffer, MAX_PATH, "%s:XL %d (%Xh)", handleBuffer, byChannel, handle);
    else if (protocol == CANProtocol::CAN_FD)
...
std::string GetSetParameter::ConvertToChannelDirection(UINT32 value)
{
    switch (value)
    {
    ...
    case LAN_DIRECTION_READ_WRITE:
        ...
        break;
    default:
        return "undefined: " + std::to_string(value);
    }
}
...
std::string GetSetParameter::ConvertToChannelFeatures(UINT32 value)
{
    ...
    if ((value & FEATURE_XL_CAPABLE) == FEATURE_XL_CAPABLE)
        if (sFeatures != "")
