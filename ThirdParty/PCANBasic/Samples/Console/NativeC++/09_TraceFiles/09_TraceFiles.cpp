...
void TraceFiles::ReadMessages()
{
    ...
    {
        if (Protocol == CANProtocol::CAN_XL)
            stsResult = CAN_ReadXL(PcanHandle, &CANMsgXL, &TimestampXL);
        else if (Protocol == CANProtocol::CAN_FD)
...
bool TraceFiles::ConfigureTrace()
{
    ...
    if (stsResult == PCAN_ERROR_OK)
    {
        ...
        if (stsResult == PCAN_ERROR_OK)
        {
            UINT32 config;
            if (TraceFileSingle)
                config = TRACE_FILE_SINGLE; // Creates one file 
            else
                config = TRACE_FILE_SEGMENTED; // Creates more files
...
void TraceFiles::ShowCurrentConfiguration()
{
    ...
    if(TraceFileDataLength)
        std::cout << "* TraceFileDataLength: True\n";
    else
        std::cout << "* TraceFileDataLength: False\n";
...
void TraceFiles::FormatChannelName(TPCANHandle handle, LPSTR buffer, CANProtocol protocol)
{
    BYTE byChannel;
