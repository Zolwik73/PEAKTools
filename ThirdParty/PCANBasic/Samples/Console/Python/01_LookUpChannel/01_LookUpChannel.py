...
class LookUpChannel():
    ...
    def __init__(self):
        """
        Create an object starts program
        """
        ...
        if self.DeviceGUID != b'':
            if self.sParameters != b'':
                self.sParameters += b', '
            self.sParameters += LOOKUP_DEVICE_GUID + b'=' + self.DeviceGUID
        ...
        if stsResult[0] == PCAN_ERROR_OK:
            ...
            if handle != PCAN_NONEBUS:
                ...
                if stsResult[0] == PCAN_ERROR_OK:
                    ...
                    isFD = (stsResult[1] & FEATURE_FD_CAPABLE) == FEATURE_FD_CAPABLE
                    isXL = (stsResult[1] & FEATURE_XL_CAPABLE) == FEATURE_XL_CAPABLE
                    print("The channel handle " + self.FormatChannelName(handle, self.GetProtocol(isFD, isXL)) + " was found")
                else:
    ...
    def ShowConfigurationHelp(self):
        ...
        print("* DeviceGUID: String value that represents a valid GUID for a PCAN device                |")
        print("                                                                                         |")
    ...
    def ShowCurrentConfiguration(self):
        ...
        print("* DeviceGUID: " + self.ConvertBytesToString (self.DeviceGUID))
        print("")
    ...
    def GetProtocol(self, isFD, isXL):
        """
        Gets the required CAN protocol
        
        Parameters:
            isFD = If device supports CAN FD
            isXL = If device supports CAN XL

        Returns:
            A CAN protocol
        """

        if not isFD and not isXL:
            return CANProtocol.CAN
        elif isFD and not isXL:
            return CANProtocol.CAN_FD
        elif not isFD and isXL:
