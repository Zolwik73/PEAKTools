...
pub fn run() {
    ...
    loop {
        read_messages();
        println!("Do you want to read again? yes[y] or any other key to close");
...
fn show_configuration_help() {
    println!("=========================================================================================");
    println!("|                           PCAN-Basic ManualRead Example                                |");
    println!("=========================================================================================");
...
fn get_msg_type_string(msg_type: TPCANMessageType) -> String {
    ...
    if (msg_type & PCAN_MESSAGE_ERRFRAME) == PCAN_MESSAGE_ERRFRAME {
        return "ERROR".to_string();
    }

    let mut s = if (msg_type & PCAN_MESSAGE_EXTENDED) == PCAN_MESSAGE_EXTENDED {
        "EXT".to_string()
    } else {
        "STD".to_string()
    };
    ...
    if (msg_type & PCAN_MESSAGE_RTR) == PCAN_MESSAGE_RTR {
        ...
        s.push_str(" [ ");
        if (msg_type & PCAN_MESSAGE_FD) == PCAN_MESSAGE_FD {
            s.push_str(" FD");
        }
        if (msg_type & PCAN_MESSAGE_BRS) == PCAN_MESSAGE_BRS {
            s.push_str(" BRS");
        }
        if (msg_type & PCAN_MESSAGE_ESI) == PCAN_MESSAGE_ESI {
            s.push_str(" ESI");
        }
        s.push_str(" ]");
    }
...
fn get_msg_type_string_xl(msg_type: TPCANMessageTypeXL, rrs_flag: bool, sec_flag: bool) -> String {
    ...
    if (msg_type & PCAN_MESSAGE_STATUS as u16) == PCAN_MESSAGE_STATUS as u16 {
        return "STATUS".to_string();
    }
    if (msg_type & PCAN_MESSAGE_ERRFRAME as u16) == PCAN_MESSAGE_ERRFRAME as u16 {
        return "ERROR".to_string();
    }

    let mut s = if (msg_type & PCAN_MESSAGE_XL) == PCAN_MESSAGE_XL {
        "XL".to_string()
    } else if (msg_type & PCAN_MESSAGE_EXTENDED as u16) == PCAN_MESSAGE_EXTENDED as u16 {
        "EXT".to_string()
    } else {
        "STD".to_string()
    };
    ...
    if (msg_type & PCAN_MESSAGE_RTR as u16) == PCAN_MESSAGE_RTR as u16 {
        ...
        if msg_type >= PCAN_MESSAGE_XL {
            ...
            if (msg_type & PCAN_MESSAGE_FD as u16) == PCAN_MESSAGE_FD as u16 {
                s.push_str(" FD");
            }
            if (msg_type & PCAN_MESSAGE_BRS as u16) == PCAN_MESSAGE_BRS as u16 {
                s.push_str(" BRS");
            }
            if (msg_type & PCAN_MESSAGE_ESI as u16) == PCAN_MESSAGE_ESI as u16 {
                s.push_str(" ESI");
            }
            s.push_str(" ]");
        }
...
fn get_data_string_raw(data: &[u8], data_length: usize) -> String {
    ...
    for b in data.iter().take(data_length) {
