...
public class CANReadThread extends Thread implements IRcvEventProcessor
{
    ...
    private Boolean readTimeStamp = true;
        	...
        	synchronized (connectedChannelCollection) {
	            for (ChannelItem item : connectedChannelCollection)
	            {
	                if ((item != MarkAllChannelItem.getInstance()) &&
	                        (item.getWorking())) {
                            callAPIFunctionReadXl(item.getHandle());
	                }
	            }
        	}
            // Sleep Time
     ...
     */
    public void callAPIFunctionReadXl(TPCANHandle handle)
    {
        //Local variables
        TPCANMsgXL canMessage = null;
        TPCANTimestampXL rcvTime = null;
        ...
        try
        {
            do
            {
                ...
                canMessage = new TPCANMsgXL();
                // Create new TimeStamp object
                rcvTime = new TPCANTimestampXL();
                //If TimeStamp is needed
                if (readTimeStamp) {
                    // We execute the "ReadXL" function of the PCANBasic
                    ret = pcanBasic.ReadXL(handle, canMessage, rcvTime);
                //If TimeStamp is not needed
                }
                else {
                    // We execute the "ReadXL" function of the PCANBasic
                    ret = pcanBasic.ReadXL(handle, canMessage, null);
                }
                ...
                if (msgRead)
                {
                    ...
                    messageID = (((long)canMessage.getType() << 32) | canMessage.getPID());
                    ...
                    synchronized (Application.token)
                    {
                        ...
                        if (readTimeStamp)
                            ...
                            dataRow.setRcvTime((TPCANTimestampXL) null);
