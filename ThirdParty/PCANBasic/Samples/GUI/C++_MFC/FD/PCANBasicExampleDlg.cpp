...
void CPCANBasicExampleDlg::OnEnKillfocusTxtlength()
{
    ...
    // Set constraints, so the dlc cannot exceed the FD range from 0 to 15
    //
    if (txtLengthInt < 0)
    {
        txtLengthInt = 0;
    }
    else if (txtLengthInt > 15)
    {
        txtLengthInt = 15;
    }
    ...
    CString dlcText;
    dlcText.Format("%d", GetLengthFromDLC(txtLengthInt));
    GetDlgItem(IDC_LALENGTH)->SetWindowText(dlcText + __T(" B."));

    // Enable data fields accordingly
    //
    EnableDisableDataFields(GetLengthFromDLC(txtLengthInt));
    UpdateData(FALSE);
}
...
void CPCANBasicExampleDlg::OnDeltaposNudlength(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    iNewVal = GetLengthFromDLC(iNewVal);
    GetDlgItem(IDC_LALENGTH)->SetWindowText(IntToStr(iNewVal) + __T(" B."));

    *pResult = 0;
    EnableDisableDataFields(iNewVal);
}
...
void CPCANBasicExampleDlg::OnBnClickedChbremote()
{
    ...
    //
    EnableDisableDataFields(nudLength.GetPos());
}
    ...
    if (stsResult == PCAN_ERROR_OK)
    {
        ...
        if (stsResult == PCAN_ERROR_OK)
            ...
            for (int i = 0; i < (int)iChannelsCount; i++)
            {
                ...
                // Checks whether the current channel is available and if it supports the FD protocols
                if ((info[i].channel_condition & PCAN_CHANNEL_AVAILABLE) == PCAN_CHANNEL_AVAILABLE &&
...
void CPCANBasicExampleDlg::OnCbnSelchangeComboparameter()
{
    ...
    labelDeviceOrDelay.SetWindowText((cbbParameter.GetCurSel() == 20) ? "Delay (µs):" : "Device ID (Hex):");
    nudParameterDevNumberOrDelay.SetPos32(0);
    txtParameterDevNumber = "0";
    UpdateData(FALSE);
}
    ...
    //
    {
        ...
        for(int i=0; i < m_LastMsgsList->GetCount(); i++)
        {
            ...
            if(msgStatus->MarkedAsUpdated)
            {
                ...
                lstMessages.SetItemText(iCurrentItem,MSG_LENGTH,IntToStr(GetLengthFromDLC(msgStatus->CANMsg.DLC)));
...
void CPCANBasicExampleDlg::InsertMsgEntry(TPCANMsgFD NewMsg, TPCANTimestampFD timeStamp)
{
    ...
    //
    {
        ...
        //
        lstMessages.SetItemText(iCurrentItem,MSG_LENGTH,IntToStr(GetLengthFromDLC(NewMsg.DLC)));
