...
class MessageStatus(object):
    def __init__(
        self, canMsg=TPCANMsgFD(), canTimestamp=TPCANTimestampFD(), listIndex=-1
    ):
        self.__m_Msg = canMsg
    ...
    def DataString(self):
        strTemp = ""
        for i in range(GetLengthFromDLC(self.__m_Msg.DLC)):
...
class PCANBasicExample(object):
    ...
    def __init__(self, parent):
        ...
        self.m_Parent.wm_title("PCAN-Basic FD Example")
    ...
    def InitializeConnectionWidgets(self):
        ...
        self.m_BitrateTXT = StringVar(
            value="f_clock_mhz=20, nom_brp=5, nom_tseg1=2, nom_tseg2=1, nom_sjw=1, data_brp=2, data_tseg1=3, data_tseg2=1, data_sjw=1"
        )
        ...
        self.txtBitrate = Entry(
            self.gbConnection, width=75, textvariable=self.m_BitrateTXT
        )
        self.txtBitrate.grid(row=1, column=2, columnspan=5, padx=10, pady=0)
    ...
    def InitializeFilteringWidgets(self):
        ...
        Label(self.gbMsgFilter, anchor=W, width=16, text="To:").grid(
            row=0, column=5, sticky=W
        )

        self.chbFilterExt = Checkbutton(
            self.gbMsgFilter,
            text="Extended Frame",
            variable=self.m_FilterExtCHB,
            command=self.chbFilterExt_CheckedChanged,
        )
        ...
        self.rdbFilterCustom = Radiobutton(
            ...
            text="Custom (expand)",
            value=2,
            variable=self.m_FilteringRDB,
        )
        self.rdbFilterCustom.grid(row=1, column=3, padx=0, pady=2, sticky=W)

        self.nudIdFrom = Spinbox(
            self.gbMsgFilter, width=10, from_=0, to=0x7FF, textvariable=self.m_IdFromNUD
        )
        self.nudIdFrom.grid(row=1, column=4, padx=0, pady=2)

        self.nudIdTo = Spinbox(
            self.gbMsgFilter, width=10, from_=0, to=0x7FF, textvariable=self.m_IdToNUD
        )
    ...
    def InitializeConfigurationWidgets(self):
        ...
        for name, value in self.m_PARAMETERS.items():
    ...
    def InitializeWritingWidgets(self):
        ...
        # Controls
        #
        Label(self.gbWriting, anchor=W, text="Data (Hex):").grid(
            row=0, columnspan=2, padx=0, column=0, sticky=W
        )

        Label(self.gbWriting, anchor=W, text=" ").grid(
            row=0, padx=0, column=16, sticky=W
        )

        Label(self.gbWriting, anchor=W, text="ID (Hex):").grid(
            row=0, padx=0, column=17, sticky=W
        )
        ...
        Label(self.gbWriting, anchor=W, width=5, text="Length:").grid(
            row=0, column=19, sticky=W
        )
        Label(self.gbWriting, anchor=W, width=5, textvariable=self.m_LengthLA).grid(
            row=1, padx=5, column=19, sticky=W
        )

        self.chbExtended = Checkbutton(
            self.gbWriting,
            text="Extended",
            variable=self.m_ExtendedCHB,
            command=self.txtID_Leave,
        )
        self.chbExtended.grid(row=2, column=17, padx=0, pady=0)

        self.chbBRS = Checkbutton(self.gbWriting, text="BRS", variable=self.m_BRSCHB)

        self.chbBRS.grid(row=2, column=18, padx=0, pady=0, sticky=W)
    ...
    def GetMsgString(self, msgStatus):
        ...
        strTemp = str(GetLengthFromDLC(msgStatus.CANMsg.DLC))
        toRet = toRet + (
    ...
    def ProcessMessageFD(self, *args):
        with self._lock:
            ...
            for msg in self.m_LastMsgsList:
                if (msg.CANMsg.ID == theMsg.ID) and (
                    msg.CANMsg.MSGTYPE == theMsg.MSGTYPE
                ):
        ...
        if result[0] == PCAN_ERROR_OK:
            ...
            for channel in result[1]:
                ...
                if (
                    ...
                    and protocol & CANProtocol.CAN_FD == CANProtocol.CAN_FD
                ):
                    items.append(
    ...
    def btnInit_Click(self):
        ...
        result = self.m_objPCANBasic.InitializeFD(
            self.m_PcanHandle, bytes(self.m_BitrateTXT.get(), "utf-8")
        )
    ...
    def btnFilterApply_Click(self):
        ...
        if self.m_FilteringRDB.get() == 2:
            ...
            #
            result = self.m_objPCANBasic.FilterMessages(
                self.m_PcanHandle,
            ...
            if result == PCAN_ERROR_OK:
                self.IncludeTextMessage(
                    "The filter was customized. IDs from {0:X} to {1:X} will be received".format(
    ...
    def WriteFrameFD(self):
        ...
        CANMsg.MSGTYPE = (
            PCAN_MESSAGE_EXTENDED if self.m_ExtendedCHB.get() else PCAN_MESSAGE_STANDARD
        )
        CANMsg.MSGTYPE |= PCAN_MESSAGE_FD.value
        CANMsg.MSGTYPE |= (
            PCAN_MESSAGE_BRS.value
            if self.m_BRSCHB.get()
            else PCAN_MESSAGE_STANDARD.value
        )

        # iLength = self.GetLengthFromDLC(CANMsg.DLC, not(CANMsg.MSGTYPE & PCAN_MESSAGE_FD.value))
        iLength = GetLengthFromDLC(CANMsg.DLC)
    ...
    def nudLength_ValueChanged(self):
        ...
        iCount = GetLengthFromDLC(int(self.m_LengthNUD.get()))
    ...
    def nudLength_Leave(self, event):
        ...
        self.m_LengthNUD.set(str(dlc))

        iCount = GetLengthFromDLC(dlc)
        self.m_LengthLA.set("%s B." % iCount)
